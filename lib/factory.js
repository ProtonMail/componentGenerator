 ...
 * @return {void}
 */
const buildComponent = ({ transformations, source, dest, fileName, body, full, exist }) => {
    gulp.src(source) // Load template to parse
        .pipe(
            tap((file) => {
                // Bind informations into the body of the template
                const content = file.contents.toString();
                const str = !body ? transformations.reduce((acc, pipe) => (acc = pipe(acc)), content) : body;
